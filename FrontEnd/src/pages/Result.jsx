...
export default function ResultPage() {
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#1E1E1E]">
        <p className="text-xl mb-4 text-gray-200">No data found. Please fill the form first.</p>
        <button
          onClick={() => navigate("/")}
  ...
  const probabilityStyles = {
    ...
    Borderline: "text-yellow-400 bg-yellow-900 border-yellow-700",
  ...
  return (
    <div className="min-h-screen bg-[#121212] py-10 px-4">
      <div className="max-w-3xl mx-auto bg-[#1E1E1E] rounded-2xl shadow-xl overflow-hidden">
        <div className="bg-[#2A2A2A] p-8 text-white text-center">
        ...
        <div className="p-8 bg-[#1E1E1E]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div className="bg-[#2A2A2A] border border-gray-700 p-6 rounded-xl text-center">
              <p className="text-xs uppercase text-gray-400 font-bold mb-1">Total Score</p>
              <p className="text-4xl font-black text-blue-400">{totalScore}</p>
            </div>

            <div className="bg-[#2A2A2A] border border-gray-700 p-6 rounded-xl text-center">
              <p className="text-xs uppercase text-gray-400 font-bold mb-1">Category</p>
              <p className="text-xl font-bold text-gray-200">{data.category || "General"}</p>
            </div>
          ...
          {/* Sectional Scores */}
          <div className="bg-[#2A2A2A] border border-gray-700 p-6 rounded-xl mb-8 shadow-sm">
            <h3 className="font-bold text-gray-200 text-lg mb-6 flex justify-between">
              Sectional Breakdown
              {data.exam === "indore" && (
                <span className={`text-xs px-2 py-1 rounded ${clearsSectionals ? 'bg-green-800 text-green-400' : 'bg-red-800 text-red-400'}`}>
              ...
              {sectionNames.map((name, index) => (
                <div key={index}>
                  <div className="flex justify-between text-sm mb-2">
                    ...
                    <span className="font-bold text-gray-200">{scores[index]} / {maxMarks[index]}</span>
                  </div>
          ...
          <div className="mt-8 flex flex-col gap-3">
            <button
              onClick={() => navigate("/")}
              className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-all shadow-md active:scale-95"
            >
